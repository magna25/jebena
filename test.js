import jebena, {ifKey,jx, int, pInt,date, minLen, maxLen, len, runEach, match, equals, email, oneOf, any} from "./src/validator.js"
...
class ExpressMock {
    ...
    async post(path, spec, callback, ops){
        await jx(spec, ops)(this.req, this.res, this.next)
...
const tests = [
    ...
    ["validate len function for type string and expect -> true", true, {password:len(5)}, {password:"55555"} ],
    ["validate len function for type string and expect -> false", false, {password:len(15)}, {password:"55555"} ],
    ["validate len function for type number and expect -> true", true, {password:len(5)}, {password:55556} ],
    ["validate len function for type number and expect -> false", false, {password:len(5)}, {password:34} ],
    ["validate minLen function for type string and expect -> true", true, {password:minLen(5)}, {password:"55555"} ],
    ["validate minLen function for type string and expect -> false", false, {password:minLen(15)}, {password:"55555"} ],
    ["validate minLen function for type number and expect -> true", true, {password:minLen(5)}, {password:555555} ],
    ["validate minLen function for type number and expect -> false", false, {password:minLen(5)}, {password:34} ],
    ["validate maxLen function for type string and expect -> true", true, {password:maxLen(5)}, {password:"55555"} ],
    ["validate maxLen function for type string and expect -> false", false, {password:maxLen(1)}, {password:"55555"} ],
    ["validate maxLen function for type number and expect -> true", true, {password:maxLen(15)}, {password:555555} ],
    ["validate maxLen function for type number and expect -> false", false, {password:maxLen(1)}, {password:34} ],
    ["validate minLen function for type array and expect -> false", false, {password:maxLen(1)}, {password:[]} ],
    ["validate minLen function for type boolen and expect -> false", false, {password:maxLen(1)}, {password:false} ],
    ["validate minLen function for type null and expect -> false", false, {password:maxLen(1)}, {password:null} ],
    ["validate minLen function for type undefined and expect -> false", false, {password:maxLen(1)}, {password:undefined} ],
    ["validate minLen function for type object<any> and expect -> false", false, {password:maxLen(1)}, {password:{}} ],
    ["validate maxLen function for type array and expect -> false", false, {password:maxLen(1)}, {password:[]} ],
    ["validate maxLen function for type boolen and expect -> false", false, {password:maxLen(1)}, {password:false} ],
    ["validate maxLen function for type null and expect -> false", false, {password:maxLen(1)}, {password:null} ],
    ["validate maxLen function for type undefined and expect -> false", false, {password:maxLen(1)}, {password:undefined} ],
    ["validate maxLen function for type object<any> and expect -> false", false, {password:maxLen(1)}, {password:{}} ],
    ["validate multiple specs ({phone:runEach(Number, len(10))}) and expect -> true", true, {phone:runEach(Number, len(10))}, {phone:1234567891}],
    ["validate multiple specs ({phone:runEach(Number, len(10))}) and expect -> false", false, {phone:runEach(Number, len(10))}, {phone:123456}],   
    ...
    ["validate email function -> expect true", true, {email:email()}, {email:"foo@bar.com"}],   
    ["validate email function -> expect false - wrong type", false, {email:email()}, {email:2}],   
    ["validate email function -> expect false - invalid email", false, {email:email()}, {email:"foobar@ccom"}],   
    ["validate email function -> expect false - invalid email", false, {email:email()}, {email:"foobarccom@"}],
    ["validate int function -> expect true", true, {age:int()}, {age:5}],
    ["validate int function -> expect false", false, {age:int()}, {age:'ss'}],
    ["validate pInt function -> expect true", true, {age:pInt()}, {age:1}],
    ["validate pInt function -> expect false", false, {age:pInt()}, {age:0}],
    ["validate date function -> expect false", false, {date: date('dd/mm/yyyy')}, {date:'50/20/2020'}],
    ["validate date function -> expect true", true, {date: date()}, {date:'10/20/2020'}],
    ["not allow missing props", false, {age:Number}, {}],   
    ["allow custom validation -> expect false", false, {age:runEach(Number, (val) => val > 20?true:false)}, {age:18}],   
    ["allow custom validation -> expect true", true, {age:runEach(Number, (val) => val > 20?true:false)}, {age:21}],   
    ...
    ["validate ifKey -- condition not met", true, {"age?":Number, title: runEach(String,len(4), ifKey("age", (val) => val > 15))}, {age:6}],   
    ["validate ifKey -- condition met", false, {"age":Number, title: runEach(String,len(4), ifKey("age", (val) => val > 5))}, {age:6}],   
